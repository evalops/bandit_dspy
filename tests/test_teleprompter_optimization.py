...
import dspy
import json
from bandit_dspy import BanditTeleprompter, bandit_metric, SecurityMetric
...
class AdaptiveLLM(dspy.LM):
    ...
    def __call__(self, messages, **kwargs):
        ...
        if any(secure_pattern in message_text.lower() for secure_pattern in 
...
def test_teleprompter_optimization():
    ...
    security_result = bandit_metric(None, prediction)
    assert security_result['score'] > 0.1  # Should be better than completely insecure code
...
def test_optimization_methods_comparison():
    ...
    for method in methods:
        ...
        prediction = compiled_program(description="a function that validates user input")
        security_result = bandit_metric(None, prediction)
...
def test_security_metric_integration():
    ...
    def strict_metric_wrapper(example, pred, trace=None):
        ...
        return {"score": result["score"], "issues": result["issues"], "issue_counts": result["issue_counts"]}
    
    teleprompter = BanditTeleprompter(
        metric=strict_metric_wrapper,
        k=1,
        num_candidates=2
    )
...
def test_edge_cases():
    ...
    small_trainset = [
        dspy.Example(description="a function", code="def func(): return 1").with_inputs('description')
    ]
    ...
    teleprompter_large_k = BanditTeleprompter(metric=bandit_metric, k=5, num_candidates=2)
    compiled_program_large_k = teleprompter_large_k.compile(student, trainset=small_trainset)
