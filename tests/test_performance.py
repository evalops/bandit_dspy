...
class TestCachingPerformance:
    ...
    def test_multiple_cache_hits(self):
        ...
        assert (
            avg_time_per_analysis < 0.01
        ), f"Cache hits too slow: {avg_time_per_analysis:.4f}s per analysis"
...
class TestOptimizationPerformance:
    ...
    def test_random_optimization_scaling(self, performance_trainset):
        ...
        for num_candidates in candidate_counts:
            ...
            compiled_program = teleprompter.compile(
                student, trainset=performance_trainset[:5]
            )
    ...
    def test_genetic_vs_random_performance(self, performance_trainset):
        ...
        start_time = time.time()
        random_program = random_teleprompter.compile(
            student, trainset=performance_trainset[:5]
        )
        ...
        start_time = time.time()
        genetic_program = genetic_teleprompter.compile(
            student, trainset=performance_trainset[:5]
        )
        ...
        assert (
            genetic_time < 60.0
        ), f"Genetic optimization too slow: {genetic_time:.3f}s"
...
class TestSecurityMetricPerformance:
    ...
    def test_metric_evaluation_speed(self, large_code_sample):
        ...
        assert (
            avg_time < 0.1
        ), f"SecurityMetric too slow: {avg_time:.4f}s per evaluation"
    ...
    def test_different_metric_configs_performance(self):
        ...
        for config in configs:
            ...
            start_time = time.time()
            for _ in range(50):
                _ = metric.evaluate(code)
            eval_time = time.time() - start_time

            avg_time = eval_time / 50
            assert (
                avg_time < 0.05
            ), f"Config {config} too slow: {avg_time:.4f}s per evaluation"
...
class TestMemoryUsage:
    ...
    def test_genetic_optimizer_memory(self):
        ...
        large_trainset = [
            dspy.Example(
                description=f"func {i}", code=f"def func_{i}(): return {i}"
            ).with_inputs("description")
            for i in range(100)
        ]
