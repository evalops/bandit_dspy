...
def test_mipro_integration():
    ...
    with patch("bandit_dspy.teleprompter.MIPRO_AVAILABLE", False):
        teleprompter = BanditTeleprompter(
            metric=create_bandit_metric(),  # Use the factory function
            optimization_method="mipro",
        )

        trainset = [
            dspy.Example(description="test", code="def test(): pass").with_inputs(
                "description"
            )
        ]
...
def test_genetic_optimizer_edge_cases():
    ...
    trainset = [
        dspy.Example(description="test1", code="def test1(): pass").with_inputs(
            "description"
        ),
        dspy.Example(description="test2", code="def test2(): pass").with_inputs(
            "description"
        ),
    ]
...
def test_genetic_crossover_edge_cases():
    ...
    trainset = [
        ...
        dspy.Example(description="test3", code="def test3(): pass").with_inputs(
            "description"
        ),
    ]
