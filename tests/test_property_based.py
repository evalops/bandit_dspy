...
simple_python_code = st.one_of(
    [
        ...
        st.builds(lambda name: f"def {name}(): return 1", python_identifiers),
...
class TestSecurityMetricProperties:
    ...
    def test_security_metric_score_bounds(
        self, severity_weight, confidence_weight, threshold
    ):
        """SecurityMetric scores should always be between 0 and 1."""
        metric = SecurityMetric(
            severity_weights={
                "HIGH": severity_weight,
                "MEDIUM": severity_weight / 2,
                "LOW": severity_weight / 4,
            },
            confidence_weights={
                "HIGH": confidence_weight,
                "MEDIUM": confidence_weight / 2,
                "LOW": confidence_weight / 4,
            },
        ...
        for code in test_codes:
            ...
            assert (
                0 <= result["score"] <= 1
            ), f"Score {result['score']} out of bounds for code: {code}"
            assert (
                result["total_penalty"] >= 0
            ), f"Penalty {result['total_penalty']} should be non-negative"
            assert (
                result["lines_of_code"] >= 0
            ), f"LOC {result['lines_of_code']} should be non-negative"
    ...
    @given(normalize=st.booleans(), threshold=st.floats(min_value=0.0, max_value=5.0))
    @settings(max_examples=20)
    def test_normalization_consistency(self, normalize, threshold):
        """Test that normalization behaves consistently."""
        metric = SecurityMetric(
            normalize_by_length=normalize, penalty_threshold=threshold
        )
    ...
    def test_run_bandit_returns_list(self, code):
        ...
        try:
            ...
            for issue in result:
                ...
                assert hasattr(issue, "confidence") or hasattr(
...
class TestSecurityMetricMonotonicity:
    ...
    @given(weight_multiplier=st.floats(min_value=1.1, max_value=10.0))
    @settings(max_examples=20)
