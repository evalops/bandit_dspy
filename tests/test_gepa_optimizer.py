...
def sample_trainset():
    ...
    return [
        dspy.Example(
            description="create a function that adds numbers",
            code="def add(a, b): return a + b",
        ).with_inputs("description"),
        dspy.Example(
            description="create a secure hash function",
            code="import hashlib\ndef hash_string(s): return hashlib.sha256(s.encode()).hexdigest()",
        ).with_inputs("description"),
        dspy.Example(
            description="create a file reader",
...
class TestParetoSelector:
    ...
    def test_add_dominated_candidate(self):
        ...
        assert selector.add_candidate(candidate1)
        ...
        assert not selector.add_candidate(candidate2)
        assert len(selector.pareto_front) == 1
    ...
    def test_add_dominating_candidate(self):
        ...
        assert selector.add_candidate(candidate2)
        assert len(selector.pareto_front) == 1
...
class TestSecurityReflector:
    ...
    def test_reflector_initialization(self, dspy_lm_fixture):
        reflector = SecurityReflector()
        assert reflector.reflection_lm == dspy_lm_fixture
...
class TestSecurityGEPAOptimizer:
    ...
    def test_optimizer_initialization(self):
        optimizer = SecurityGEPAOptimizer(
            max_iterations=5, population_size=4, reflection_frequency=2
        )
    ...
    def test_initialize_population(self, sample_trainset):
        ...
        population = optimizer._initialize_population(
            "Generate secure code", sample_trainset[:1], sample_trainset
        )
    ...
    def test_configure_program(self, dspy_lm_fixture):
        optimizer = SecurityGEPAOptimizer()
        student = DummyModule()
...
class TestGEPATeleprompter:
    ...
    def test_teleprompter_initialization(self, security_metric):
        teleprompter = GEPATeleprompter(
            metric=security_metric, max_iterations=3, population_size=2
        )
...
class TestGEPAIntegration:
    ...
    def test_end_to_end_small_scale(self, sample_trainset, security_metric):
        ...
        optimizer = SecurityGEPAOptimizer(
            max_iterations=1, population_size=2, reflection_frequency=1
        )
