...
class SecurityReflector(dspy.Module):
    ...
    def _get_reflection_signature(self):
        ...
        class SecurityReflectionSignature(dspy.Signature):
            ...
            generated_code = dspy.InputField(
                desc="The Python code generated by the LLM"
            )
            security_issues = dspy.InputField(
                desc="List of security issues found by static analysis"
            )
            current_instruction = dspy.InputField(
                desc="Current instruction prompt used"
            )
            ...
            execution_trace = dspy.InputField(
                desc="Details of how the code was generated"
            )

            # Outputs
            problem_diagnosis = dspy.OutputField(
                desc="Analysis of why security issues occurred"
            )
            root_cause_analysis = dspy.OutputField(
                desc="Deeper analysis of prompt/example weaknesses"
            )
            improved_instruction = dspy.OutputField(
                desc="Improved instruction prompt focusing on security"
            )
            improved_guidelines = dspy.OutputField(desc="Enhanced security guidelines")
            example_improvements = dspy.OutputField(
                desc="Suggestions for better few-shot examples"
            )
            security_trade_offs = dspy.OutputField(
                desc="Analysis of security vs functionality trade-offs"
            )
    ...
    def forward(self, trajectory_data: Dict[str, Any]) -> SecurityReflection:
        ...
        security_issues_text = self._format_security_issues(
            trajectory_data.get("security_issues", [])
        )
        examples_text = self._format_examples(
            trajectory_data.get("few_shot_examples", [])
        )
    ...
    def _format_security_issues(self, issues: List[Dict[str, Any]]) -> str:
        ...
        for issue in issues:
            formatted.append(
                f"- {issue.get('test_id', 'Unknown')}: {issue.get('description', 'No description')}"
            )
        return "\n".join(formatted)
    ...
    def _format_examples(self, examples: List[Any]) -> str:
        ...
        for i, example in enumerate(examples[:3]):  # Limit to first 3 for brevity
            if hasattr(example, "description") and hasattr(example, "code"):
                formatted.append(
                    f"Example {i+1}: {example.description} -> {example.code[:100]}..."
                )
        return "\n".join(formatted)
    ...
    def _extract_improvements(self, reflection_result) -> List[str]:
        ...
        if hasattr(reflection_result, "improved_instruction"):
            improvements.append(
                f"Instruction: {reflection_result.improved_instruction}"
            )
...
class ParetoSelector:
    ...
    def __init__(self, objectives: Optional[List[str]] = None):
        ...
        self.pareto_front: List[GEPACandidate] = []
        self.dominated_solutions: List[GEPACandidate] = []
    ...
    def _compare_candidates(self, a: GEPACandidate, b: GEPACandidate) -> int:
        ...
        for objective in self.objectives:
            a_score = a.scores.get(objective, 0) if a.scores is not None else 0
            b_score = b.scores.get(objective, 0) if b.scores is not None else 0
    ...
    def get_best_candidates(self, k: int = 5) -> List[GEPACandidate]:
        ...
        ideal_point = {
            obj: max(float(c.scores.get(obj, 0)) if c.scores is not None else 0.0 for c in self.pareto_front)
        ...
        for candidate in self.pareto_front:
            distance = (
                sum(
                    (ideal_point[obj] - (candidate.scores.get(obj, 0) if candidate.scores is not None else 0)) ** 2
                    for obj in self.objectives
                )
                ** 0.5
            )
...
class SecurityGEPAOptimizer:
    ...
    def __init__(
        ...
        self.reflection_history: List[SecurityReflection] = []
        self.best_candidates: List[GEPACandidate] = []
    ...
    def optimize(
        ...
        population = self._initialize_population(
            seed_instruction or "Generate secure Python code for the given task.",
            seed_examples or [], # Provide an empty list if None
            trainset
        )
        ...
        for iteration in range(self.max_iterations):
            ...
            # Track progress
            best_candidate = max(
                population, key=lambda c: c.scores.get("security_score", 0) if c.scores is not None else 0
            )
            optimization_history["iterations"].append(
                {
                    "iteration": iteration,
                    "best_security_score": best_candidate.scores.get(
                        "security_score", 0
                    ) if best_candidate.scores is not None else 0,
    ...
    def _initialize_population(
        self, seed_instruction: str, seed_examples: List[Any], trainset: List[Any]
    ) -> List[GEPACandidate]:
        ...
        seed_candidate = GEPACandidate(
            instruction_prompt=seed_instruction
            or "Generate secure Python code for the given task.",
            few_shot_examples=seed_examples
            or random.sample(trainset, min(3, len(trainset))),
    ...
    def _evaluate_candidate(
        ...
        for example in sample_examples:
            ...
            trajectory = {
                ...
                "code": (
                    prediction.code if hasattr(prediction, "code") else str(prediction)
                ),
        ...
        performance_score = 1.0 - (
            len(str(trajectories[0]["code"])) / 1000
        )  # Simple proxy
    ...
    def _perform_reflection(
        self, iteration_results: List[Tuple[GEPACandidate, Dict]]
    ) -> List[SecurityReflection]:
        """Perform LLM-based reflection on iteration results."""
        reflections = []

        # Focus on worst-performing candidates for reflection
        sorted_results = sorted(
            iteration_results, key=lambda x: x[0].scores.get("security_score", 0) if x[0].scores is not None else 0
        )
        ...
        for candidate, trajectory in sorted_results[:3]:
            if trajectory["trajectories"]:
                ...
                reflection_data = {
                    ...
                    "security_score": candidate.scores.get("security_score", 0) if candidate.scores is not None else 0,
                    "performance_score": candidate.scores.get("performance_score", 0) if candidate.scores is not None else 0,
                    "instruction": candidate.instruction_prompt,
                    "few_shot_examples": candidate.few_shot_examples,
                    "security_guidelines": candidate.security_guidelines,
    ...
    def _generate_candidates_from_reflection(
        self, reflections: List[SecurityReflection], trainset: List[Any]
    ) -> List[GEPACandidate]:
    ...
    def _extract_improved_guidelines(self, reflection: SecurityReflection) -> str:
        ...
        return (
            "1. Never hardcode passwords or API keys\n"
    ...
    def _mutate_candidate(
        self, base_candidate: GEPACandidate, mutation_id: int, trainset: List[Any]
    ) -> GEPACandidate:
        ...
        if mutation_type == "instruction":
            ...
            new_candidate.instruction_prompt = (
                random.choice(variations) + " the given task."
            )
            ...
            new_candidate.performance_config["max_tokens"] = random.choice(
                [300, 400, 500, 600]
            )
    ...
    def _configure_program(
        self, student_program: dspy.Module, candidate: GEPACandidate
    ) -> dspy.Module:
        """Configure student program with candidate settings."""
        configured_program = (
            student_program.deepcopy()
            if hasattr(student_program, "deepcopy")
            else student_program
        )
        ...
        for name, module in configured_program.named_modules():
            if isinstance(module, dspy.Predict):
                ...
                original_signature_cls = type(module.signature)

                # Create enhanced signature class
                class SecurityEnhancedSignature(original_signature_cls): # type: ignore
                    pass

                # Modify the docstring to include security guidelines
                if (
                    hasattr(module.signature, "__doc__")
                    and module.signature.__doc__
                ):
                    enhanced_doc = f"{module.signature.__doc__}\n\nSECURITY GUIDELINES:\n{candidate.security_guidelines}\n\nINSTRUCTION: {candidate.instruction_prompt}"
                else:
    ...
    def _create_optimized_program(
        self, student_program: dspy.Module, best_candidate: GEPACandidate
    ) -> dspy.Module:
        ...
        optimized_program._gepa_metadata.update(
            {
                ...
                "final_scores": best_candidate.scores if best_candidate.scores is not None else {},
        ...
        if (
            hasattr(optimized_program, "set_examples")
            and best_candidate.few_shot_examples
        ):
...
class GEPATeleprompter:
    ...
    def compile(self, student, *, trainset) -> dspy.Module:
        ...
        optimized_program, history = self.optimizer.optimize(
            student_program=student, trainset=trainset, security_metric=self.metric
        )

        print("GEPA optimization completed.")
        print(
            f"Final Pareto front size: {len(self.optimizer.pareto_selector.pareto_front)}"
        )
