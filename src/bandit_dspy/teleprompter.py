...
class GeneticOptimizer:
    ...
    def __init__(self, population_size=20, generations=10, mutation_rate=0.1, elite_ratio=0.2):
    ...
    def optimize(self, trainset, k, evaluator):
        ...
        final_scores = [(evaluator(individual), individual) for individual in population]
...
class BanditTeleprompter(Teleprompter):
    ...
    def _compile_bayesian(self, student, train_subset, val_subset):
        ...
        best_examples = self.genetic_optimizer.optimize(train_subset, optimal_k, evaluator)
        optimizer = LabeledFewShot(k=len(best_examples))
        return optimizer.compile(student, trainset=best_examples)
    ...
    def _compile_mipro(self, student, train_subset, val_subset):
        ...
        mipro_optimizer = MIPRO(
            metric=security_metric_wrapper,
            **self.mipro_config
        )
